...
double MinTimeStrategy::CalculateLinkTime(LINKID linkId, double dataAmount) {
    // 获取链路对象
    auto& link = net->m_vAllLinks[linkId];

    // 获取当前可用的密钥数量
    double availableKeys = link.GetAvaialbeKeys();

    // 获取密钥生成速率（通过GetQKDRate方法获取）
    double keyRate = link.GetQKDRate();
...
bool MinTimeStrategy::Route(NODEID sourceId, NODEID sinkId, std::list<NODEID>& nodeList, std::list<LINKID>& linkList) {
    // 默认数据量为1.0
    return Route(sourceId, sinkId, nodeList, linkList, 1.0);
}
